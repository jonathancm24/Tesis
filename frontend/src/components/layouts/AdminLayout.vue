...
const menuItems = [
  { 
    ...
    icon: 'fas fa-home', 
    to: { name: 'HomeAdmin' },
    color: 'success' // Verde institucional
  },
  { 
    name: 'AdminUsers', 
    label: 'Usuarios', 
...
@media (min-width: 992px) {
  .sidebar {
    ...
    background: linear-gradient(135deg, var(--soft-green) 0%, #e0f2f1 100%);
  }
}
...
.btn-profile {
  ...
  align-items: center;
  transition: all 0.2s ease;
  box-shadow: 0 2px 4px rgba(129, 199, 132, 0.25);
}
...
.admin-layout.dark-mode .btn-hamburger {
  background: var(--primary-green);
}

.admin-layout.dark-mode .btn-header-control {
  background: var(--bg-primary);
  border-color: var(--border-color);
}

.admin-layout.dark-mode .btn-header-control:hover {
  background: var(--soft-green);
  color: var(--dark-gray);
}
